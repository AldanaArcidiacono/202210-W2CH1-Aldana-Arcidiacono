...
const getArrayNumbers = () => {
    let userNumbers = prompt(
        "Escribe todos los números con los que deeses hacer tus calculos, separandolos con un guión medio ('-')"
    )
        .trim()
        .split('-')
...
const calculatorFunction = () => {
    let startCalculation = confirm(
        "Deseas realizar calculos? Si lo deseas, toca 'Aceptar', de lo contrario toca 'Cancelar'."
    );
    ...
    while (startCalculation === true) {
        ...
        if (myArgs.length === 1) {
            myOperationsArray.push(
                `La raíz cuadrada de ${myArgs} es ${roundNumber(
                    squareRoot(...myArgs),
                    3
                )}`
            );
            startCalculation = confirm(myOperationsArray);
            startCalculation = confirm(
                "Deseas realizar más cálculos? Si lo deseas, toca 'Aceptar', de lo contrario toca 'Cancelar'."
            );
        } else {
            myOperationsArray.push(
                `Los números que seleccionaste son: ${myArgs}.\n`,
                `El resultado de la suma de tus números es: ${roundNumber(
                    addition(...myArgs),
                    3
                )}\n`,
                `El resultado de la resta de tus números es: ${roundNumber(
                    subtract(...myArgs),
                    3
                )}\n`,
                `El resultado de la multiplicación de tus números es: ${roundNumber(
                    multiplication(...myArgs),
                    3
                )}\n`,
                `El resultado de la división de tus números es: ${roundNumber(
                    division(...myArgs),
                    3
                )}`
            );
