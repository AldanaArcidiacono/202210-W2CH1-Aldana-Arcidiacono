...
const categoryMember = (userName) => {
    let adminOrUser = prompt(
        "Para continuar, escribe cuál es tu tipo de usuario: 'ADMIN' o 'USER'. Si quieres salir presiona 'cancelar'"
    )
        .toUpperCase()
        .trim();
    if (adminOrUser === null) {
        alert(
            `Gracias por usar nuestro programa ${userName}! Vuelve pronto!✈️🛩`
        );
    }
    while (adminOrUser !== 'ADMIN' && adminOrUser !== 'USER') {
        adminOrUser = prompt(
            `Perdona ${userName}, no te hemos entendido. Por favor escribe tu tipo de usuario: 'ADMIN' o 'USER'`
        )
            .toUpperCase()
            .trim();
    }
...
const checkAdminAction = (userName) => {
    const doAdminAction = confirm(
        "Presiona 'aceptar' si deseas crear o eliminar vuelos \n Presiona 'cancelar' para salir del programa."
    );
    if (!doAdminAction) {
        alert(
            `Gracias por usar nuestro programa ${userName}! Vuelve pronto!✈️🛩`
...
const onlyDeleteFlights = (userName) => {
    ...
    const wantToDelete = confirm(
        "Presiona 'aceptar' si deseas eliminar un vuelo o 'cancelar' de lo contrario."
    );
    if (wantToDelete) {
...
const onlyCreateFlights = (userName) => {
    ...
    const wantToCreate = confirm(
        "Presiona 'aceptar' si deseas crear un nuevo vuelo o 'cancelar' de lo contrario."
    );
    if (wantToCreate) {
...
const askAdminAction = (userName) => {
    const askForAction = confirm(
        `${userName}, selecciona 'aceptar' para crear un vuelo, o 'cancelar' para eliminar un vuelo`
    );
    if (askForAction) {
        createFlights(flights);
    } else {
...
const createFlights = () => {
    ...
    while (flightIdExist) {
        newFlight.id = +prompt(
            'El número introducido de ID ya corresponde a uno de nuestros vuelos. Por favor, ingresa un ID válido'
        );
    ...
    while (newFlight.to === null || newFlight.to === '') {
        newFlight.to = prompt(
            'Por favor escribe la ciudad de destino.',
            'Málaga'
        ).trim();
    }
    ...
    while (newFlight.from === null || newFlight.from === '') {
        newFlight.from = prompt(
            'Por favor escribe la ciudad de origen.',
            'Madrid'
        ).trim();
    }
    ...
    while (
        ...
        newFlight.cost = +prompt(
            'El valor introducido no es un número válido. Por favor ingresa el costo del vuelo.',
            80
        );
    }
    newFlight.scale = confirm(
        "Presiona 'aceptar' si el vuelo tiene escalas o 'cancelar' si no realiza escalas."
    );
...
const deleteFlights = () => {
    alert(
        'A continuación te mostraremos los vuelos disponibles. Luego te pediremos que insertes el número del vuelo que desees eliminar.'
    );
    infoFlights(flights);
    let flightToDelete = +prompt(
        'Inserta el número del vuelo que deseas eliminar.'
    );
    while (
        ...
        flightToDelete = +prompt(
            'El número introducido no corresponde a ningún ID de nuestros vuelos. Por favor, ingresa un ID válido'
        );
    }
...
const userAction = (userName) => {
    let askCost = +prompt(
        `${userName}, ingresa tu presupuesto para el vuelo. O presiona 'cancelar' para salir del programa.`
    );
    if (askCost === null) {
        alert(
            `Gracias por usar nuestro programa ${userName}! Vuelve pronto!✈️🛩`
    ...
    while (isNaN(askCost) || askCost === null || askCost === 0) {
        askCost = +prompt(
            'El valor introducido no es un número válido. Por favor ingresa tu presupuesto para el vuelo.'
        );
    }
    ...
    alert(
        `Estos son los vuelos que se ajustan a tu presupuesto:\n ${priceOfFlights}`
    );
};

const newUserAction = (userName) => {
    const newAction = confirm(
        "Deseas ingresar otro presupuesto? Selecciona 'aceptar' si deseas continuar, o 'cancelar' para salir"
    );
    if (!newAction) {
        alert(
            `Gracias por usar nuestro programa ${userName}! Vuelve pronto!✈️🛩`
